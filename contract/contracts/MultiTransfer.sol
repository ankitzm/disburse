...
contract MultiTransfer is OwnerIsCreator {
    ...
    // Function to transfer tokens to multiple recipients based on percentages
    function transferTokensToMultipleAddresses(
    ...
    {
        require(
            _destinationChainSelectors.length == _receivers.length &&
        ...
        for (uint256 i = 0; i < _receivers.length; i++) {
            transferTokensPayLINK(
                _destinationChainSelectors[i],
                _token,
                _amount * _percentages[i] / 100,
                _receivers[i]
            );
        }
    }
    ...
    // Modified function to handle transfer to multiple addresses
